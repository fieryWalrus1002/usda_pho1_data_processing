...
def get_genotype(filename: str) -> str:
    plant_genotype = filename.split("\\")[-1].split("-")[0]
...
def get_plant_id(filename: str) -> str:

    if "-" in filename:
        plant_id = filename.split("-")[-1].split(".")[0]
    else:
        plant_id = 0
    return plant_id
...
if __name__ == "__main__":
    ...
    for i, filename in enumerate(files):
        ...
        plant_id = get_plant_id(filename)
        ...
        Pm, P, Po = get_p_vals(pmax_df, pm_start=450, pm_end=500)
        ...
        for i, label in enumerate(unique_labels):
            ...
            if "PMAX" not in label:
                ...
                # put a P next to the P line, a P' next to the P' line, etc
                ax.text(pm_start - 50, Pm, "Pm", fontsize=14)
                ax.text(pm_start - 50, Pmp, "Pmp", fontsize=14)
                ax.text(pm_start - 50, P, "P", fontsize=14)
                ax.text(pm_start - 50, Po, "Po", fontsize=14)
            else:
                ax.set_ylabel("Fraction of P700 oxidized", fontsize=14)
                ax.hlines(
                    1, 0, 1300, color="red", linestyle="--"
